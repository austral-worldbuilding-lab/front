...
export default function ProjectFilesList({projectId: propProjectId, files, loading, error}: ProjectFilesListProps) {
    const [fileToDelete, setFileToDelete] = useState<ProjectFile | null>(null);
    const [isDialogOpen, setIsDialogOpen] = useState(false);
    const {projectId: paramProjectId} = useParams<{ projectId: string }>();

    const projectId = propProjectId || paramProjectId;
    const {removeFile, isDeleting} = useProjectFiles(projectId!);
        ...
        <div>
          ...
          {error && (
              <p className="text-red-500">
                {typeof error === "string" ? error : "Error loading files"}
              </p>
          )}
